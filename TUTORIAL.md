...
| 一键整片模型 | 单价 | 单次时长上限 | 30 秒约合 |
|---|---|---|---|
| Seedance 2.5（当前默认） | $0.134/秒 | 30 秒 | 约 $4.0 |
| Wan 3.0 | $0.04/秒 | 30 秒 | 约 $1.2 |
| Wan 3.0 Prime | $0.061/秒 | 30 秒 | 约 $1.8 |
| MiniMax H3 | $0.038/秒 | 15 秒 | 15 秒约 $0.6 |

> **分辨率档位会明显改变单价。** 原生 480p / 720p / 1080p 和 `-sr` / `-esr` 超分档在平台上是**不同产品、不同定价**，超分档更贵。上面是基准价，实际以你所选平台和档位的实时价为准。
